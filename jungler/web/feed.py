...
def feeds():
    feed_list = Feed.query.order_by(Feed.write_time.desc(), Feed.id.desc()).limit(30).all()
...
def additional_feeds():
    ...
    feeds_ = Feed.query.filter(
        Feed.id != last_feed.id,
        Feed.write_time > last_feed.write_time
    ).order_by(
        Feed.write_time.desc(), Feed.id.desc()
    ).limit(30).all()
